...
def get_args():
    ...
    parser.add_argument('--p_schema_table_config', required=True, type=str, help="Location for csv file containing schem table mapping.")
    ...
    parser.add_argument('--p_child_code_file', required=True, help="xxxxxxxxxxxxxxxxxxxx")
...
def process_tasks(task):
    ...
    task = task.split(":")
    table = task[1].split(".")
    logger.info('Starting chunk %s for %s.%s', task[0],table[0],table[1])
    ...
    pythonscript = 'python3 ' + os.path.abspath(os.getcwd()) + '/' + p_child_code_file + ' "' + s3path + '" "' + \
        task[2] + '" "' + p_username + '" "' + password + '" "' + p_oracle_host + '" "' + \
    ...
    if int(o.decode("utf-8")) == 0:
        logger.info('Finished chunk %s for %s.%s', task[0],table[0],table[1])
    else:
        logger.error('Error processing chunk %s for %s.%s', task[0],table[0],table[1])
...
def main():
    ...
    #ora_conn = cx_Oracle.connect(user=list(USER.keys())[
    #                             0], password=password, dsn=p_oracle_host+"/"+p_service+":"+str(p_port))
    ...
    with open(schema_table_config) as csv_file:
        ...
        logger.info(
            'Reading chunks from DBA_PARALLEL_EXECUTE_CHUNKS to execute task in parallel.')
        ...
        logger.info('Processed ETL scripts queue')
